...
def compute_signals(feat, date_str, station, nwp_data):
    ...
    # 2. ceiling_visibility_trend (NEW SIGNAL for step 4 - ceiling and visibility trends)
    ceiling_rate_of_change = None
    visibility_rate_of_change = None
    
    # Check for valid trend in ceiling ft
    if ceiling_ft is not None and prev_ceiling is not None:
        ceiling_change = ceiling_ft - prev_ceiling
        # Normalize typical ceiling changes (-5000 to 5000ft range)
        ceiling_rate_of_change = max(-1.0, min(1.0, ceiling_change / 1000.0))
        
    # Check for valid trend in visibility_mi
    if visibility_mi is not None and prev_visibility is not None:
        visibility_change = visibility_mi - prev_visibility
        # Normalize visibility changes (-10 to 10 mi range)
        visibility_rate_of_change = max(-1.0, min(1.0, visibility_change / 10.0))
        
    # Combine both ceiling and visibility trends
    if ceiling_rate_of_change is not None and visibility_rate_of_change is not None:
        combined_trend = (ceiling_rate_of_change + visibility_rate_of_change) / 2.0
        signals["ceiling_visibility_trend"] = max(-1.0, min(1.0, combined_trend))
    elif ceiling_rate_of_change is not None:
        signals["ceiling_visibility_trend"] = ceiling_rate_of_change
    elif visibility_rate_of_change is not None:
        signals["ceiling_visibility_trend"] = visibility_rate_of_change
    else:
        signals["ceiling_visibility_trend"] = 0.0
...
signals_list = ["ceiling_visibility_trend","pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly"]
...
print("\n=== 500-ROUND RANDOM SEARCH (10 signals with ceiling_visibility_trend) ===")
...
for trial in range(500):  # Increase trials to 500 as required
    ...
    s = sum(raw)
    w = [x/s for x in raw]
    weights = dict(zip(["ceiling_visibility_trend","pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly"], w))
...
print(f"\n=== BEST 10-SIGNAL CONFIGURATION (500 trials with ceiling_visibility_trend) ===")
