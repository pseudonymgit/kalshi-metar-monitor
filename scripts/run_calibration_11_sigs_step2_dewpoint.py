    ...
    for station, obs_list in stations.items():
        ...
        if prev_date and station in metar_daily.get(prev_date, {}):
            ...
            if prev_obs:
                ...
                prev_dews = [o["dewpoint_c"] for o in prev_obs if o.get("dewpoint_c") is not None]
                ...
                feat["prev_dewpoint_c"] = sum(prev_dews)/len(prev_dews) if prev_dews else None
            else:
                ...
                feat["prev_ceiling_ft"] = feat["prev_visibility_mi"] = feat["prev_dewpoint_c"] = None
        else:
            feat["prev_temp_c"] = feat["prev_pressure_mb"] = feat["prev_sea_level_pressure_mb"] = None
            feat["prev_wind_kt"] = feat["prev_wind_gust_kt"] = feat["prev_wind_dir"] = None
            feat["prev_ceiling_ft"] = feat["prev_visibility_mi"] = feat["prev_dewpoint_c"] = None
...
def compute_signals(feat, date_str, station, nwp_data):
    ...
    prev_dewpoint = feat.get("prev_dewpoint_c")
...
signals_list = ["dewpoint_depression_delta","pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly"]
...
print("\n=== 500-ROUND RANDOM SEARCH (10 signals with dewpoint_depression_delta) ===")
...
for trial in range(500):  # Increase trials to 500 as required
    ...
    s = sum(raw)
    w = [x/s for x in raw]
    weights = dict(zip(["dewpoint_depression_delta","pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly"], w))
...
print(f"\n=== BEST 10-SIGNAL CONFIGURATION (500 trials with dewpoint_depression_delta) ===")
