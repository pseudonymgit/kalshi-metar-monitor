#!/usr/bin/env python3
"""
Minimal 11-signal patch on top of the verified 9-signal working script.
Adds only the reversion flag (reversion=0 → +0.9, reversion=1 → -0.8).
Uses a PARALLEL dict for reversion so the original (bucket, prior) tuple is untouched.
"""
...
print("=== 11-SIGNAL MINIMAL PATCH (reversion added, coverage preserved) ===")
print("Stations:", STATIONS)
print("Trials: 500 | Risk limit: 8 consecutive losses")

# NWP (exact same as working 9-signal script)
...
# METAR + settlement (exact same query as working 9-signal script)
metar_conn = sqlite3.connect(METAR_DB)
settlements = {}
reversions = {}   # PARALLEL dict — does not touch the original tuple
...
"""):
    ...
    reversions[(date, station)] = reversion if reversion is not None else 0
...
"""):
    ...
    metar_daily[date][station].append({
        "temp_c": temp_c, "pressure_mb": pressure_mb, "sea_level_pressure_mb": slp_mb,
        "wind_kt": wind_kt or 0, "wind_gust_kt": gust_kt, "dewpoint_c": dewpoint_c,
...
def compute_signals(feat, date_str, station, nwp_data, reversion=0):
    ...
    # 11th signal — reversion (minimal addition)
    if reversion == 0:
        signals["reversion"] = 0.9
    elif reversion == 1:
        signals["reversion"] = -0.8
    else:
        signals["reversion"] = 0.0

    return signals

# Signals (original 9 + dtr_anomaly + reversion)
SIGNALS = ["pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection",
...
weights = {s: 1.0/11.0 for s in SIGNALS}
threshold = 0.08
...
for key in sorted(settlements.keys()):
    ...
    reversion = reversions.get(key, 0)
