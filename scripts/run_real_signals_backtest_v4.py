...
print("=== REAL 7-SIGNAL BACKTEST v4 (cloud_cover + NWP placeholders wired) ===")
...
for station, ts, date_utc, temp_c, pressure_mb, wind_kt, dewpoint_c, ceiling_ft, visibility_mi in rows:
    ...
    # 6. cloud_cover_modulation (real logic from ceiling + visibility)
    if ceiling_ft is not None and visibility_mi is not None:
        if ceiling_ft > 10000 and visibility_mi > 10:
            signals["cloud_cover_modulation"] = 0.6
        elif ceiling_ft < 3000 or visibility_mi < 3:
            signals["cloud_cover_modulation"] = -0.7
        else:
            signals["cloud_cover_modulation"] = 0.0
    else:
        signals["cloud_cover_modulation"] = 0.2

    # 7. forecast_disagreement (placeholder – we have NWP data but not yet joined)
    signals["forecast_disagreement"] = 0.0   # will be wired when we join nwp_forecasts.db
