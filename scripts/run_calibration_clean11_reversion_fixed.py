#!/usr/bin/env python3
"""
Calibration v5 (fixed) — Clean 11-signal set with Reversion flag
Exact copy of the working 9-signal structure + minimal addition of reversion.
...
print("=== CLEAN 11-SIGNAL CALIBRATION (reversion flag, coverage fixed) ===")
print("Stations:", STATIONS)
print("Trials: 500 | Risk limit: 8 consecutive losses")

# NWP multi-model (same as working 9-signal script)
...
# METAR + settlement (EXACT same query as 9-signal working version + reversion)
...
"""):
    # Store reversion alongside the 2-tuple (compatible with existing code)
    settlements[(date, station)] = {
        "bucket": bucket,
        "prior": prior,
        "reversion": reversion if reversion is not None else 0
    }
...
# Collapse + prior-day (exact structure from working 9-signal script)
metar_features = {}
...
for key in sorted(settlements.keys()):
    ...
    sb = settlements[key]
    settlement_bucket = sb["bucket"]
    prior_bucket = sb["prior"]
    reversion = sb["reversion"]
...
for trial in range(500):
    ...
    s = sum(raw)
    w = [x/s for x in raw]
    weights = dict(zip(["pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly","dtr_anomaly","reversion"], w))
...
print(f"\n=== BEST 11-SIGNAL CONFIGURATION (500 trials, coverage fixed) ===")
