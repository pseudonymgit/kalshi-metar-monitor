...
for date_utc, stations in metar_daily.items():
    for station, obs_list in stations.items():
        ...
        # Add two-day ago information for pressure tendency and gust
        if prev2_date and station in metar_daily.get(prev2_date, {}):
            ...
            if prev2_obs:
                ...
                prev2_gusts = [o["wind_gust_kt"] for o in prev2_obs if o["wind_gust_kt"] is not None]
                feat["prev2_pressure_mb"] = sum(prev2_press)/len(prev2_press) if prev2_press else None
                feat["prev2_gust_kt"] = sum(prev2_gusts)/len(prev2_gusts) if prev2_gusts else None
            else:
                feat["prev2_pressure_mb"] = None
                feat["prev2_gust_kt"] = None
        else:
            feat["prev2_pressure_mb"] = None
            feat["prev2_gust_kt"] = None
...
def compute_signals(feat, date_str, station, nwp_data):
    ...
    prev2_gust_kt = feat.get("prev2_gust_kt")  # Extra lag for 2-day gust change
    ...
    # 2. gust_change_2day (NEW SIGNAL for step 3 - 2-day wind gust change)
    if prev_gust is not None and prev2_gust_kt is not None and gust_kt is not None:
        # Calculate 2-day gust change
        gust_2day_change = gust_kt - prev2_gust_kt
        # Normalizing considering typical gust kts (0-40)
        signals["gust_change_2day"] = max(-1.0, min(1.0, gust_2day_change / 15.0))
    elif prev_gust is not None and gust_kt is not None:
        # Fallback to 1-day gust change
        gust_1day_change = gust_kt - prev_gust
        signals["gust_change_2day"] = max(-1.0, min(1.0, gust_1day_change / 10.0))
    else:
        signals["gust_change_2day"] = 0.0
...
signals_list = ["gust_change_2day","pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly"]
...
print("\n=== 500-ROUND RANDOM SEARCH (10 signals with gust_change_2day) ===")
...
for trial in range(500):  # Increase trials to 500 as required
    ...
    s = sum(raw)
    w = [x/s for x in raw]
    weights = dict(zip(["gust_change_2day","pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly"], w))
...
print(f"\n=== BEST 10-SIGNAL CONFIGURATION (500 trials with gust_change_2day) ===")
