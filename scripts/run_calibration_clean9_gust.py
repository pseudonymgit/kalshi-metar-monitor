...
# Collapse + prior-day (including gust)
metar_features = {}
...
# --- Baseline with 9 signals ---
print("\n=== BASELINE (9 signals, equal weights, threshold 0.08) ===")
weights = {s: 1.0/9.0 for s in ["pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly"]}
threshold = 0.08
...
print("\n=== 200-ROUND RANDOM SEARCH (9 signals) ===")
best = {"acc": 0, "pnl": 0, "weights": None, "threshold": None, "trades": 0}

for trial in range(200):
    raw = [random.uniform(0.35, 2.3) for _ in range(9)]
    s = sum(raw)
    w = [x/s for x in raw]
    weights = dict(zip(["pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly"], w))
...
print(f"\n=== BEST 9-SIGNAL CONFIGURATION (200 trials) ===")
