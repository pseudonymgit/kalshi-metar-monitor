#!/usr/bin/env python3
"""
Calibration v4 (fixed coverage) — Clean 10-signal set with DTR anomaly lever
Mirrors the 9-signal structure exactly, adds DTR (MAX-MIN temp_f same day).
...
print("=== CLEAN 10-SIGNAL CALIBRATION (DTR anomaly, coverage fixed) ===")
...
# METAR + settlement (exact same pattern as 9-signal script)
...
# Collapse + prior-day (including DTR and all other fields)
metar_features = {}
    ...
    for station, obs_list in stations.items():
        ...
        if prev_date and station in metar_daily.get(prev_date, {}):
            ...
            if prev_obs:
                ...
                feat["prev_ceiling_ft"] = feat["prev_visibility_mi"] = feat["prev_dtr_f"] = None
        else:
            feat["prev_temp_c"] = feat["prev_pressure_mb"] = feat["prev_sea_level_pressure_mb"] = None
            feat["prev_wind_kt"] = feat["prev_wind_gust_kt"] = feat["prev_wind_dir"] = None
            feat["prev_ceiling_ft"] = feat["prev_visibility_mi"] = feat["prev_dtr_f"] = None
...
metar_conn.close()
print(f"Data ready: {len(settlements)} settlement epochs, {len(metar_features)} METAR days (full coverage)")
...
def compute_signals(feat, date_str, station, nwp_data):
    ...
    # 2. gaussian_v2 (real day-to-day temp delta)
...
# --- Baseline with 10 signals (full coverage) ---
...
print("\n=== 500-ROUND RANDOM SEARCH (10 signals, full coverage) ===")
...
print(f"\n=== BEST 10-SIGNAL CONFIGURATION (500 trials, full coverage) ===")
