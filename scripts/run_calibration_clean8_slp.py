#!/usr/bin/env python3
"""
Calibration v2 — Clean 8-signal set with sea_level_pressure_mb lever
Includes all previous real signals + new SLP anomaly signal.
...
print("=== CLEAN 8-SIGNAL CALIBRATION (sea_level_pressure lever added) ===")
...
# METAR + settlement (now pulls sea_level_pressure_mb)
...
for row in metar_conn.execute("""
    SELECT station, date_utc, temp_c, pressure_mb, sea_level_pressure_mb,
           wind_speed_kt, dewpoint_c, wind_direction_deg, ceiling_ft, visibility_mi
    FROM metar_observations
    WHERE station IN ('KNYC','KLAX','KMDW','KBOS','KATL','KSFO','KSEA')
      AND temp_c IS NOT NULL AND pressure_mb IS NOT NULL
"""):
    station, date, temp_c, pressure_mb, slp_mb, wind_kt, dewpoint_c, wind_dir, ceiling_ft, visibility_mi = row
...
# Collapse + prior-day features (including slp)
metar_features = {}
    ...
    for station, obs_list in stations.items():
        ...
        if prev_date and station in metar_daily.get(prev_date, {}):
            ...
            if prev_obs:
                ...
                feat["prev_wind_kt"] = feat["prev_wind_dir"] = None
...
def compute_signals(feat, date_str, station, nwp_data):
    ...
    # 8. slp_anomaly (NEW lever - sea_level_pressure_mb)
...
# --- Baseline with 8 signals ---
print("\n=== BASELINE (8 signals, equal weights, threshold 0.08) ===")
weights = {s: 1.0/8.0 for s in ["pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly"]}
threshold = 0.08
...
print("\n=== 200-ROUND RANDOM SEARCH ===")
best = {"acc": 0, "pnl": 0, "weights": None, "threshold": None, "trades": 0}

for trial in range(200):
    raw = [random.uniform(0.4, 2.2) for _ in range(8)]
    s = sum(raw)
    w = [x/s for x in raw]
    weights = dict(zip(["pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly"], w))
...
print(f"\n=== BEST 8-SIGNAL CONFIGURATION (200 trials) ===")
