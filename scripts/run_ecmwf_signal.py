#!/usr/bin/env python3
"""
ADVANCE Signal Runner: ECMWF IFS Bias-Corrected (P1.x operational)

Fetches 3-day ECMWF IFS forecasts for all active stations via Open-Meteo,
applies rolling bias correction, and logs results for the pipeline.

Usage:
    python3 scripts/run_ecmwf_signal.py [--stations KNYC,KLAX] [--output json]
...
logger = logging.getLogger("run_ecmwf_signal")


def collect_forecasts(stations: List[str], signal: ECMWFBiasCorrectedSignal) -> Dict:
    """Fetch ECMWF IFS forecasts for all stations and store in DB."""
    results = {}
    for station in stations:
        ...
        try:
            ...
            if extremes.get("max_f") is not None:
                ...
                logger.info(
                    f"{station}: ECMWF max={extremes['max_f']}°F, "
                    f"min={extremes['min_f']}°F, confidence={extremes['confidence']:.3f}"
                )
            else:
                results[station] = {"error": "no_forecast"}
                logger.warning(f"{station}: No ECMWF forecast returned")
        except Exception as e:
            results[station] = {"error": str(e)}
            logger.error(f"{station}: ECMWF fetch failed: {e}")
...
def generate_trade_signals(results: Dict) -> List[Dict]:
    """
    Convert ECMWF IFS forecasts into trade signals for the pipeline.
    ...
    for station, data in results.items():
        ...
        if max_f > 75:
            signals.append({
                ...
                "reason": f"ECMWF bias-corrected max {max_f}°F above 75°F threshold",
                "signal_name": "ecmwf_bias_corrected",
            })
        elif max_f < 65:
            signals.append({
                "station": station,
                "direction": "down",
                "confidence": min(data["confidence"] + 0.05, 0.95),
                "reason": f"ECMWF bias-corrected max {max_f}°F below 65°F threshold",
                "signal_name": "ecmwf_bias_corrected",
            })
...
def main():
    parser = argparse.ArgumentParser(description="ECMWF IFS Bias-Corrected Signal Runner")
    ...
    logger.info(f"Running ECMWF signal for {len(stations)} stations")
    ...
    if not args.collect_only:
        ...
        logger.info(f"Generated {len(signals)} trade signals from ECMWF data")
        for s in signals:
