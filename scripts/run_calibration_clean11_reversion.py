#!/usr/bin/env python3
"""
Calibration v5 — Clean 11-signal set with Reversion flag lever
All 10 previous real meteorological signals + reversion_occurred from settlement table.
...
print("=== CLEAN 11-SIGNAL CALIBRATION (reversion flag added) ===")
...
# METAR + settlement (now also pulls reversion_occurred)
...
# Collapse + prior-day (exact same structure as 10-signal v2)
metar_features = {}
...
metar_conn.close()
print(f"Data ready: {len(settlements)} settlement epochs, {len(metar_features)} METAR days (reversion included)")
...
def compute_signals(feat, date_str, station, nwp_data, reversion=0):
    ...
    # 11. reversion (NEW — reversion_occurred flag from settlement table)
    # Historical edge: reversion=0 → UP (high accuracy), reversion=1 → DOWN
    if reversion == 0:
