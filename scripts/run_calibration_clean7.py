#!/usr/bin/env python3
"""
Focused calibration on the clean 7-signal set (all real levers wired).
Goal: tune ensemble weights + decision threshold for higher accuracy + fewer early loss streaks.
Risk guardrails active at consecutive_loss_limit=8 (diagnostic).
"""
...
print("=== CLEAN 7-SIGNAL CALIBRATION (weights + threshold) ===")
print("Stations:", STATIONS)
print("Risk limit: 8 consecutive losses (diagnostic)")

# --- Load NWP (multi-model) ---
...
"""):
    metar_daily[date][station].append({
        ...
        "ceiling_ft": ceiling_ft, "visibility_mi": visibility_mi
    })
    ...
    for station, obs_list in stations.items():
        ...
        if prev_date and station in metar_daily.get(prev_date, {}):
            ...
            if prev_obs:
                ...
                feat["prev_temp_c"] = feat["prev_wind_kt"] = feat["prev_wind_dir"] = None
...
# --- Signal computation function (same logic as clean7) ---
...
# --- Baseline (equal weights, threshold 0.08) ---
print("\n=== BASELINE (equal weights, threshold 0.08) ===")
weights = {s: 1.0/7.0 for s in ["pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement"]}
threshold = 0.08
...
for key in sorted(settlements.keys()):
    ...
    ensemble = sum(signals[s] * weights[s] for s in signals) / sum(weights.values())
...
print("\n=== CALIBRATION RUN (simple random search on weights + threshold) ===")
print("Running 50 random configurations...")

best = {"acc": 0, "pnl": 0, "weights": None, "threshold": None, "trades": 0}

for trial in range(50):
    # random weights (dirichlet-like)
    raw = [random.uniform(0.5, 2.0) for _ in range(7)]
    s = sum(raw)
    w = [x/s for x in raw]
    weights = dict(zip(["pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement"], w))
    threshold = random.uniform(0.03, 0.12)
    ...
    acc = (correct / total_trades * 100) if total_trades > 50 else 0  # ignore tiny runs
    if acc > best["acc"] and total_trades > 50:
