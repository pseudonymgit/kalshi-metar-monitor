...
def compute_signals(feat, date_str, station, nwp_data):
    ...
    # 2. reversion_flag (NEW SIGNAL for step 6 - use the parallel dict approach)
    # Calculate a simple indicator that could suggest reversion conditions
    # Using just the current basic direction of the main signals
    
    # Check if various factors suggest extreme conditions (possibility of reversion)
    pressure_extreme = abs((pressure_mb - 1013.0) / 25.0) > 0.8 if pressure_mb is not None else False
    temp_change_extreme = abs(temp_c - prev_temp) > 8.0 if temp_c is not None and prev_temp is not None else False
    wind_extreme = (wind_kt > 25.0 or ((prev_wind_kt or 0) > 25.0)) if wind_kt is not None else False
    dp_depression_high = (temp_c - dewpoint_c > 15.0) if temp_c is not None and dewpoint_c is not None else False
    
    # Count how many extreme conditions we detect
    extreme_condition_count = sum([pressure_extreme, temp_change_extreme, wind_extreme, dp_depression_high])
    
    # If there are multiple extremes, reversion is possible
    if extreme_condition_count >= 3:
        # High extremeness indicates potential for reversion
        signals["reversion_flag"] = -0.8 # Reversion happens
    elif extreme_condition_count == 0:
        # All conditions normal, lean toward no reversion
        signals["reversion_flag"] = 0.9
    else:
        # Mixed conditions - neutral
        signals["reversion_flag"] = 0.0
...
signals_list = ["reversion_flag","pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly"]
...
print("\n=== 500-ROUND RANDOM SEARCH (10 signals with reversion_flag) ===")
...
for trial in range(500):  # Increase trials to 500 as required
    ...
    s = sum(raw)
    w = [x/s for x in raw]
    weights = dict(zip(["reversion_flag","pressure","gaussian_v2","calendar_climatology","goldilocks","wind_advection","cloud_cover_modulation","forecast_disagreement","slp_anomaly","gust_anomaly"], w))
...
print(f"\n=== BEST 10-SIGNAL CONFIGURATION (500 trials with reversion_flag) ===")
