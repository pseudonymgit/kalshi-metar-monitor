...
class SignalResult:
    ...
    def compute_metrics(self):
        ...
        # Brier score with decomposition: BS = (F - O)² where F=forecast, O=outcome
        brier_sum = 0
        reliability = 0
        resolution = 0
        uncertainty = 0
...
def signal_late_day_momentum(station, date, max_temp, settlement, prior):
    """
    Edge P1.2: Late-day METAR plateau/slope detection for same-day contracts.
    
    Analyze last-day-of-trading data for momentum signals relevant to same-day trading.
    """
    # This signal attempts to detect late-day temperature trends indicating momentum vs. plateauing
    # In production, we'd access hourly METAR data in 5PM-11PM UTC range
    # Since our data may not contain late-day, we'll create a proxy based on day-to-day settlement behavior
    
    if settlement is None or prior is None:
        return None, 0.0, 0.5
    
    # If latest settlement is trending in direction significantly different from prior settlement,
    # might suggest continued momentum  
    change = settlement - prior
    trend_strength = abs(change)
    
    if trend_strength > 4:  # Strong move in one direction  
        # Bet on momentum continuation
        direction = 'up' if change > 0 else 'down'
        confidence = 0.58 + min(0.2, trend_strength / 20.0)  # Higher confidence with stronger trends
        return direction, confidence, 0.55 + trend_strength / 50.0
    elif 1.5 < trend_strength <= 4:  # Moderate move
        # Follow the trend but with moderate confidence
        direction = 'up' if change > 0 else 'down'
        return direction, 0.56, 0.53
    else:
        # Weak movement or reversion - harder to predict direction with momentum vs reversion
        return 'up' if settlement > 68 else 'down', 0.48, 0.5  # Back to climatology baseline
...
def main():
    ...
    signal_functions = {
        ...
        'late_day_momentum': lambda s, d, m, se, pr: signal_late_day_momentum(s, d, m, se, pr),
    ...
    print("\nNote: This script integrates NWP data compatibility and regime analysis.")
