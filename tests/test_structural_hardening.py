...
class StructuralHardeningTests(unittest.TestCase):
    ...
    @patch("core.metar_monitor.ensure_state_loaded")
    @patch("core.metar_monitor.get_default_config", return_value={})
    def test_simulation_alert_generated_blocked_before_send_surfaces_upstream_gate(self, *_mocks):
    ...
    def test_process_event_alert_generated_blocked_before_send_reports_upstream_blocking_details(
        self,
        _emit_transition,
        _commit_state,
        mock_read_state,
    ...
    @patch("core.metar_monitor.ensure_state_loaded")
    @patch("core.metar_monitor.get_default_config", return_value={})
    def test_simulation_generated_alert_without_delivery_result_marks_invariant_violation(self, *_mocks):
        with patch("core.metar_monitor._ingest_obs", return_value=(1, 1)):
            result = metar_monitor._simulate_temperature_for_testing("KDEN", 71.1, allow_alert_delivery=True)

        self.assertEqual(result["alerts_generated"], 1)
        self.assertTrue(result["delivery_requested"])
        self.assertFalse(result["delivery_attempted"])
        self.assertEqual(result["delivery_blocking_stage"], "invariant_violation")
        self.assertEqual(result["delivery_blocking_reason"], "MISSING_DELIVERY_RESULT_FOR_GENERATED_ALERT")
    ...
    def test_empty_market_fetch_writes_cache_entry(self, mock_get, *_mocks):
        ...
        with patch.dict("core.kalshi_monitor._SERIES_EVENTS_CACHE", {}, clear=True):
            kalshi_monitor.build_structured_snapshot("KDEN", {"HIGH"})
    ...
    @patch(
        ...
        side_effect=[
            ...
            {"markets": [{"ticker": "KXHIGHDEN-26MAR12-T80"}]},
            {"markets": []},
            {"markets": [{"ticker": "KXHIGHDEN-26MAR12-T81"}]},
        ],
    )
    def test_series_events_cache_reuses_recent_events_payload(self, mock_get, *_mocks):
        with patch.dict("core.kalshi_monitor._SERIES_EVENTS_CACHE", {}, clear=True):
            kalshi_monitor.build_structured_snapshot("KDEN", {"HIGH"})
            kalshi_monitor.build_structured_snapshot("KDEN", {"HIGH"})
    ...
    def test_series_events_cache_fetches_when_entry_is_stale(self, mock_get, *_mocks):
        with patch.dict(
            "core.kalshi_monitor._SERIES_EVENTS_CACHE",
            {
                "KXHIGHDEN": {
                    "events": [{"event_ticker": "KXHIGHDEN-26MAR11", "status": "active"}],
                    "fetched_at_utc": "2026-03-12T11:58:00+00:00",
                }
            },
            clear=True,
        ):
            kalshi_monitor.build_structured_snapshot(
                "KDEN",
                {"HIGH"},
    ...
    @patch("core.kalshi_monitor._station_local_kalshi_date_token", return_value="26MAR12")
    def test_select_event_ticker_for_series_returns_none_without_exact_station_day_match(self, *_mocks):
        selected = kalshi_monitor._select_event_ticker_for_series(
            events=[
                {"event_ticker": "KXHIGHDEN-26MAR11", "status": "open"},
                {"event_ticker": "KXHIGHDEN-26MAR13", "status": "active"},
                {"event_ticker": "KXOTHERDEN-26MAR12", "status": "open"},
            ],
            station="KDEN",
            series_ticker="KXHIGHDEN",
            observation_time_utc="2026-03-12T06:00:00+00:00",
        )
    ...
    def test_build_snapshot_does_not_fetch_wrong_day_series_event(self, mock_get, *_mocks):
        with patch.dict("core.kalshi_monitor._SERIES_EVENTS_CACHE", {}, clear=True):
        ...
        self.assertEqual(kalshi_monitor._LAST_HYDRATION_EXECUTION["KDEN"]["raw_market_count"], 0)
        self.assertEqual(kalshi_monitor._LAST_HYDRATION_EXECUTION["KDEN"]["filtered_market_count"], 0)
        self.assertEqual((kalshi_monitor.get_cached_series_markets("KXHIGHDEN") or {}).get("markets"), [])

    @patch("core.kalshi_monitor._station_local_kalshi_date_token", return_value="26MAR12")
    def test_filter_structured_markets_uses_observation_timestamp_for_date_token(self, mock_date_token):
        kalshi_monitor._filter_structured_markets(
            [{"ticker": "KXHIGHDEN-26MAR12-B80", "status": "open"}],
            "KDEN",
            {"HIGH"},
            observation_time_utc="2026-03-12T06:00:00+00:00",
        )
        ...
        self.assertEqual(
            mock_date_token.call_args.kwargs.get("observation_time_utc"),
            "2026-03-12T06:00:00+00:00",
        )

    @patch("core.kalshi_monitor._filter_structured_markets", return_value=[])
    def test_snapshot_from_cache_forwards_observation_timestamp(self, mock_filter):
        with patch.dict("core.kalshi_monitor._SERIES_BY_STATION", {"KDEN": "KXHIGHDEN"}, clear=True):
            with patch.dict(
                "core.kalshi_monitor._SERIES_MARKETS_CACHE",
                {"KXHIGHDEN": {"markets": []}},
                clear=True,
            ):
                kalshi_monitor.build_structured_snapshot_from_cache(
                    "KDEN",
                    {"HIGH"},
                    observation_time_utc="2026-03-12T06:00:00+00:00",
                )
        ...
        self.assertEqual(
            mock_filter.call_args.kwargs.get("observation_time_utc"),
            "2026-03-12T06:00:00+00:00",
        )
...
class HydrationRecoverySafetyGateTests(unittest.TestCase):
    ...
    @patch("app._hydration_recovery_historical_station_registry", return_value=set())
    @patch("app._canonical_live_station_universe", return_value={"stations": [], "configured_stations": set(), "watchlist_stations": set()})
    @patch("app._is_non_production_environment", return_value=False)
    @patch("app._current_kalshi_execution_domain", return_value="production")
    def test_recovery_refuses_unknown_station(self, _domain, _env, _live_universe, _historical_registry):
        response = self.client.post("/ops/hydration-recovery", json={"station": "KDEN"})

        self.assertEqual(response.status_code, 403)
        payload = response.get_json()
        self.assertEqual(payload["status"], "refused")
        self.assertEqual(payload["reason"], "station_not_known_to_system")

    @patch("app._hydration_recovery_historical_station_registry", return_value={"KDEN"})
    @patch("app._canonical_live_station_universe", return_value={"stations": [], "configured_stations": set(), "watchlist_stations": set()})
    ...
    @patch("app._is_non_production_environment", return_value=False)
    @patch("app._current_kalshi_execution_domain", return_value="production")
    def test_recovery_allows_station_in_historical_registry(
        self,
        _domain,
        _env,
        mock_enqueue,
        ...
        mock_worker.return_value = {"status": "hydrated", "station": "KDEN"}

        response = self.client.post("/ops/hydration-recovery", json={"station": "KDEN"})
        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertEqual(payload["status"], "executed")
        self.assertEqual(payload["allow_reason"], "historical_registry")
        mock_enqueue.assert_called_once_with("KDEN", reason="ops_hydration_recovery")


    @patch("app._hydration_recovery_historical_station_registry", return_value=set())
    @patch("app._canonical_live_station_universe", return_value={"stations": ["KDEN"], "configured_stations": set(), "watchlist_stations": set()})
    @patch("app.hydration_queue_snapshot")
    @patch("app.process_hydration_queue_worker")
    @patch("app.enqueue_station_hydration")
    @patch("app._is_non_production_environment", return_value=False)
    @patch("app._current_kalshi_execution_domain", return_value="production")
    def test_bounded_recovery_executes_only_in_production_domain(self, _domain, _env, mock_enqueue, mock_worker, mock_queue, _live_universe, _historical_registry):
        mock_queue.side_effect = [
            {
                "queue": [],
                "queue_depth": 0,
        ...
        self.assertEqual(payload["allow_reason"], "live_discovered")
    ...
    @patch("app._hydration_recovery_historical_station_registry", return_value=set())
    @patch("app._canonical_live_station_universe", return_value={"stations": [], "configured_stations": {"KDEN"}, "watchlist_stations": set()})
    @patch("app.hydration_queue_snapshot")
    @patch("app.process_hydration_queue_worker")
    @patch("app.enqueue_station_hydration")
    @patch("app._is_non_production_environment", return_value=False)
    @patch("app._current_kalshi_execution_domain", return_value="production")
    def test_recovery_bounded_limit_is_clamped(self, _domain, _env, mock_enqueue, mock_worker, mock_queue, _live_universe, _historical_registry):
        mock_queue.side_effect = [
            {
                "queue": [],
                "queue_depth": 0,
        ...
        payload = response.get_json()
        self.assertEqual(payload["status"], "executed")
        self.assertEqual(payload["allow_reason"], "configured")
        self.assertEqual(payload["bounded_limit"], 3)
        mock_enqueue.assert_called_once_with("KDEN", reason="ops_hydration_recovery")
        self.assertEqual(mock_worker.call_count, 3)

    @patch("app._hydration_recovery_historical_station_registry", return_value=set())
    @patch("app._canonical_live_station_universe", return_value={"stations": [], "configured_stations": set(), "watchlist_stations": {"KLAX"}})
    @patch("app.hydration_queue_snapshot")
    @patch("app.process_hydration_queue_worker")
    @patch("app.enqueue_station_hydration")
    @patch("app._is_non_production_environment", return_value=False)
    @patch("app._current_kalshi_execution_domain", return_value="production")
    def test_debug_recovery_get_path_uses_station_eligibility_logic(
        self,
        _domain,
        _env,
        mock_enqueue,
        ...
        mock_queue.side_effect = [
            ...
            },
            {
                "queue": ["KLAX"],
                "queue_depth": 1,
                "queued_stations": ["KLAX"],
                "backoff_until": {},
                "backoff_stations": [],
                "last_hydration_request_ts": 3.0,
            },
        ]
        mock_worker.return_value = {"status": "hydrated", "station": "KLAX"}

        response = self.client.get("/debug/run-hydration-recovery?station=KLAX")
        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertEqual(payload["status"], "executed")
        self.assertEqual(payload["allow_reason"], "configured")
