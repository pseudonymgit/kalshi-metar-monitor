...
class RuntimeAuthoritySnapshotEndpointTests(unittest.TestCase):
    ...
    def test_runtime_authority_snapshot_returns_bounded_read_only_payload(self, *_mocks):
        ...
        self.assertEqual(payload["hydration_snapshot"]["source_authority"], "runtime_authority_hydration_snapshot")
        ...
        self.assertEqual(payload["hydration_execution"]["source_authority"], "retained_last_hydration_execution_snapshot")
        self.assertEqual(payload["hydration_execution"]["stations"]["KDEN"]["raw_market_count"], 24)
        self.assertEqual(payload["hydration_queue"]["source_authority"], "live_hydration_queue_snapshot")
        self.assertEqual(payload["hydration_queue"]["value"]["queue_depth"], 1)
        self.assertEqual(payload["hydration_queue"]["value"]["backoff_stations"], ["KPHL"])
        self.assertEqual(payload["hydration_queue"]["value"]["stations_in_backoff"], 1)
        self.assertEqual(payload["hydration_queue"]["value"]["next_backoff_expiry"], 123.0)
        self.assertEqual(payload["hydration_stall_signal"]["source_authority"], "derived_hydration_stall_signal")
        self.assertEqual(payload["hydration_stall_signal"]["value"]["hydration_reason"], "hydration_ready")
        self.assertEqual(payload["hydration_stall_signal"]["value"]["hydration_cache_not_written"], False)
        self.assertEqual(payload["hydration_stall_signal"]["value"]["transitions_seen_today"], 2)
        self.assertEqual(payload["hydration_stall_signal"]["value"]["alerts_sent_today"], 0)
        self.assertEqual(payload["hydration_stall_signal"]["value"]["hydration_stall_condition"], False)
    ...
    def test_runtime_authority_snapshot_does_not_trigger_live_kalshi_calls(self, *_mocks):
        ...
        self.assertEqual(payload["hydration_execution"], {"source_authority": "retained_last_hydration_execution_snapshot", "stations": {}})
        self.assertEqual(payload["hydration_queue"]["value"]["queue_depth"], 0)
        self.assertEqual(payload["hydration_queue"]["value"]["stations_in_backoff"], 0)
        self.assertEqual(payload["hydration_queue"]["value"]["next_backoff_expiry"], None)
        self.assertEqual(payload["hydration_stall_signal"]["value"]["hydration_stall_condition"], False)
        self.assertIn("system_health", payload)
    ...
    def test_runtime_authority_snapshot_does_not_mutate_hydration_execution(self, *_mocks):
        ...
        with patch("app.get_last_hydration_execution_snapshot", return_value=execution_snapshot):
            ...
            payload = response.get_json()
            self.assertEqual(payload["hydration_stall_signal"]["value"]["hydration_reason"], "hydration_cache_not_written")
