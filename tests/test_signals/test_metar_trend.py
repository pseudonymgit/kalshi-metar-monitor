...
from core.signals.metar_trend_signal import MetarTrendSignal
...
class TestMetarTrendSignal:
    """Test suite for MetarTrendSignal."""

    def test_signal_can_be_imported(self):
        """Test 1: Signal can be imported from its module."""
        sig = MetarTrendSignal()
        assert sig is not None
        assert sig.name == "metar_trend"
    ...
    def test_constructor_takes_reasonable_arguments(self):
        ...
        sig = MetarTrendSignal(db_path=None)
        assert sig is not None
        sig2 = MetarTrendSignal()
