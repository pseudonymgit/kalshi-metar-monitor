"""Tests for cooling_failure signal."""
import math
import sys
...
from core.signals.cooling_failure_signal import CoolingFailureSignal
...
class TestCoolingFailureSignal:
    """Test suite for CoolingFailureSignal."""

    def test_signal_can_be_imported(self):
        """Test 1: Signal can be imported from its module."""
        sig = CoolingFailureSignal()
        assert sig is not None
        assert sig.name == "cooling_failure"
    ...
    def test_constructor_takes_reasonable_arguments(self):
        ...
        sig = CoolingFailureSignal(db_path=None)
        assert sig is not None
        sig2 = CoolingFailureSignal()
