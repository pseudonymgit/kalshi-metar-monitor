...
class ObservabilityPipelineTruthEndpointTests(unittest.TestCase):
    ...
    def test_deterministic_for_identical_inputs(
        ...
        mock_get_transition_history.return_value = [{"transition_type": "settlement_up", "timestamp": "2030-01-01T12:05:00+00:00"}]
        mock_get_latest_market_eval.return_value = {"KDEN": {"market_eligibility_runtime": {"eligible_markets_count": 2}}}
        mock_get_recent_alerts.return_value = [{"station": "KDEN", "alert_type": "composed_alert_sent", "created_utc": "2030-01-01T12:10:00+00:00"}]
        mock_get_hydration_snapshot.return_value = {"KDEN": {"cache_valid": True, "status": "cache_valid"}}
    ...
    @patch("app.get_recent_alerts")
    def test_read_only_does_not_mutate_alert_history(
        self,
        mock_get_recent_alerts,
        _mock_get_state,
