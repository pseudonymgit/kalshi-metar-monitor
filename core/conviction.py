...
class ConvictionScorer:
    ...
    def compute_signal_agreement_score(
        ...
        for i, (name, direction, conf) in enumerate(signal_list):
            ...
            weight = weights[i] if i < len(weights) else 1.0 / n_signals
        ...
        return total_agreement / total_weight
    ...
    def compute_calibration_state(self, historical_predictions: List[Tuple[float, bool]]) -> Dict[str, float]:
        ...
        for bin_data in bins:
            ...
            ece_term = (len(bin_data) / total_examples) * abs(avg_acc - avg_prob)
            ece_sum += ece_term
            
            reliability_sum += (len(bin_data) / total_examples) * ((avg_acc - avg_prob)**2)
        
        sharpness = statistics.stdev([p for p, _ in historical_predictions]) if len(historical_predictions) > 1 else 0.0
        avg_conf = sum([p for p, _ in historical_predictions]) / len(historical_predictions)
