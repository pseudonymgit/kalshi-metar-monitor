...
Comprehensive backtesting of Phase 3 predictions against historical settlement epochs.

Metrics:
1. Directional accuracy (did temperature go the predicted direction?)
2. Magnitude error (how far off was the projection?)
3. Confidence calibration (do high-confidence predictions outperform low-confidence ones?)
4. Win rate by station, by market type, by confidence band

The backtest simulates "what if" predictions by using each closed epoch as a query epoch
and finding analogs in the prior history to project the settlement_bucket.
"""
...
from core.p3_feature_extractor import extract_features_from_epoch, FeatureVector
from core.p3_match_engine import find_similar_epochs, get_top_analogs
from core.p3_trajectory_tracer import trace_all_trajectories
...
class PredictionResult:
    ...
    # Ground truth (actual settlement that occurred)
    actual_bucket: int
    actual_direction: str  # 'up', 'down', 'flat'
    actual_jump_magnitude: int
    ...
    # Trajectory analysis (actual trajectory from analogs)
    analog_trajectory: List[int]  # Sequence of buckets observed after analog epoch
    trajectory_direction: Optional[str]  # 'up', 'down', 'flat' of analog's subsequent settlement
    trajectory_match: Optional[bool]  # Did analog's trajectory match prediction?
...
class BacktestSummary:
    ...
    total_valid_analogs: int  # Predictions with ≥3 analogs
...
class Phase3BacktestEngine:
    ...
    def get_epochs_by_station_and_date(self, station: str, local_date: str) -> List[Dict]:
        """Get all epochs for a station on a specific date."""
        self.cursor.execute("""
            SELECT 
                id, station, market_type, local_trading_date,
                settlement_bucket, prior_settlement_bucket,
    ...
    def simulate_prediction(self, query_epoch: Dict, corpus: List[Dict]) -> PredictionResult:
        """
        ...
        try:
            ...
            if has_min_analogs and top_analogs:
                ...
                if analog_prior_buckets:
                    ...
                    if avg_analog_settlement > avg_analog_prior:
                        ...
                        predicted_bucket = query_epoch.get('settlement_bucket', 0) + 5  # Simple offset
            ...
            actual_bucket = query_epoch.get('settlement_bucket', 0)
            prior_bucket = query_epoch.get('prior_settlement_bucket')
            actual_direction = self.determine_direction(actual_bucket, prior_bucket)
            ...
            if has_min_analogs:
                ...
                if analog_trajectory:
                    ...
                    if len(analog_trajectory) >= 2:
                        avg_analog_traj = statistics.mean(analog_trajectory)
            ...
            magnitude_error = abs(actual_bucket - predicted_bucket)
            distance_from_prediction = abs(actual_bucket - predicted_bucket)
            ...
            return PredictionResult(
                ...
                actual_jump_magnitude=query_epoch.get('settlement_jump_magnitude', 0),
    ...
    def run_backtest(self, start_date: str = None, end_date: str = None) -> List[PredictionResult]:
        ...
        would have been made at that time using only prior history.
        """
        ...
        for (station, market_type), epochs in station_market_epochs.items():
            ...
            for i in range(warmup_period, len(epochs)):
                ...
                if len(closed_prior_epochs) >= self.MIN_ANALOGS:
                    result = self.simulate_prediction(query_epoch, closed_prior_epochs)
                    if result:
    ...
    def calculate_summary(self, results: List[PredictionResult]) -> BacktestSummary:
        ...
        station_stats = defaultdict(lambda: {
            ...
            'avg_magnitude_error': 0, 'median_magnitude_error': 0
        })
        ...
        for station in station_stats:
            ...
            if stats['total'] > 0:
                ...
                station_errors = [r.magnitude_error for r in results if r.station == station]
                stats['median_magnitude_error'] = statistics.median(station_errors) if station_errors else 0
...
def main():
    ...
    db_paths = [
        ...
        "/home/node/.openclaw/workspace/prototypes/weather-engine-source-backup-20260627-1719/core/alerts.db",
    ...
    try:
        ...
        print("-" * 40)
        print(f"{'Station':<8} {'Total':>6} {'Correct':>8} {'Accuracy':>10} {'Avg Err':>8} {'Median':>8}")
        print("-" * 52)
        ...
        for station in sorted(summary.station_stats.keys()):
            ...
            print(f"{station:<8} {stats['total']:>6} {stats['correct']:>8} "
                  f"{stats['accuracy']:>10.2%} {stats['avg_magnitude_error']:>8.2f} {stats['median_magnitude_error']:>8.2f}")
        ...
        print("-" * 40)
        print(f"{'Market':<8} {'Total':>6} {'Correct':>8} {'Accuracy':>10}")
        print("-" * 32)
        
        for mt in sorted(summary.market_type_stats.keys()):
            stats = summary.market_type_stats[mt]
            print(f"{mt:<8} {stats['total']:>6} {stats['correct']:>8} {stats['accuracy']:>10.2%}")
        ...
        output_path = "/tmp/p3_backtest_results.json"
