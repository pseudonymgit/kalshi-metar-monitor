...
class LiquidityGate:
    """Promoted from scripts/run_b6_liquidity_gate.py — filters low-liquidity markets."""
    def __init__(self, db_path: str = None):
        self.risk_manager = RiskManager()
        self.db_path = db_path or os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "metar_backfill.db")

    def evaluate(self, station: str, date: str, signal_edge: float, direction: str = None) -> bool:
        """Return True if trade passes liquidity gate (market is liquid enough)."""
        conditions = get_market_conditions(station, date, self.db_path)
        trade_quality = calculate_trade_quality_score(direction, signal_edge, signal_edge, conditions)
        return trade_quality.get("passes_gate", False)

    def record_trade(self, is_profitable: bool):
        self.risk_manager.update_after_trade(is_profitable)
